...
export const ConnectionBanner: React.FC<ConnectionBanner.Props> = ({ connectedServer, onPress }) => {
    ...
    const handleSubmit = (event: GestureResponderEvent) => {
        ...
        if (!ipAddress.match(ipCheck)) {
            console.log('ip address did not meet requirements');
            event.preventDefault();
            setError(true);
            return;
        }
            ...
            <DialogContent>
                <DialogHeader>
                    ...
                    <>
                        ...
                        />
                        {error ? <Animated.View entering={FlipInXDown} exiting={FlipOutXDown} style={{flexDirection: 'row', alignItems: 'flex-start', alignContent: 'flex-start', gap: 4}}><Info color='red' size={16} style={{marginTop: 3}} /><Label nativeID='ip-error' style={{ color: 'red' }}>The ip you submitted is invalid, please resubmit with a valid ip</Label></Animated.View> : null}
                    </>
