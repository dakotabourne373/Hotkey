...
export const generateUnusedHotkey = (
  tree: SavedHotkeys,
  maxDepth = 4,
): KEYS[] | null => {
  function recurse(
    currentTree: SavedHotkeys,
    depth: number,
    prefix: KEYS[],
  ): KEYS[] | null {
    ...
    for (const key of ALL_KEYS) {
      ...
      if (!childNode || (!("icon" in childNode) && !("desc" in childNode)))
        return [...prefix, key];
...
export const mergeNewHotkey = (
  tree: SavedHotkeys,
  keys: KEYS[],
  parameters: Omit<HotkeyNode, "isSynced">,
) => {
  function addRecursive(
    currentTree: SavedHotkeys,
    remainingKeys: KEYS[],
  ): SavedHotkeys | HotkeyNode {
    ...
    for (const key in obj) {
      if (key !== "icon" && key !== "desc") {
        ...
        if (value && typeof value === "object")
          recurse(value as HotkeyNode, [...path, key as KEYS]);
      }
    }
  }
...
export const updateHotkey = (
  tree: SavedHotkeys | HotkeyNode,
  keys: KEYS[],
  updates: Partial<HotkeyNode>,
): SavedHotkeys | HotkeyNode => {
  if (keys.length === 0) {
    return {
      ...tree,
  ...
  const subTree =
    typeof currentNode === "object" && currentNode !== null ? currentNode : {};
