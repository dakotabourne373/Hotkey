...
import { AlertTriangle, CheckCheck, Info, Pencil, Plus, SquareStack, } from "lucide-react-native";
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    fetchData().then((data) => {
      if (isSubscribed) {
        ...
        setHotkeys(convertStoredHotkeys(data).sort((a, b) => a.index - b.index));
      }
    })
  ...
  const handleFinishedSorting = (newList: HotkeyData[]) => {
    ...
    setHotkeys(newList.map((item, index) => {
  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <FlashList
          ...
          <ContextMenu style={{ flexGrow: 1, height: '100%' }}>
            <ContextMenuTrigger asChild onLongPress={() => {
              ...
              <Button
                ...
                style={{ flex: 1 / 4, margin: 4, borderRadius: 10, alignItems: 'center', alignContent: 'center', justifyContent: 'space-around', backgroundColor: '#3c3f44', height: '100%', pointerEvents: 'box-only' }}
              >
        ...
        ListHeaderComponent={
          <>
            <ConnectionBanner connectedServer={ip} isConnected={isConnected} onPress={connectToServer} />
            <View style={{ flexDirection: 'row-reverse', justifyContent: 'space-between', padding: 16 }}>
              <Button style={{ borderRadius: 10, padding: 2, paddingHorizontal: 4, alignContent: 'center', justifyContent: 'center' }} onPress={() => {
                impactAsync(ImpactFeedbackStyle.Heavy);
